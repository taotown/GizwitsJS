...
if(!i)return void this._sendError(this.onSubscribeDevice,ERROR_CODE.GIZ_SDK_DEVICE_DID_INVALID,arguments.callee.name+": invaild did",e.did)
...
s.setInt16(t,d),t+=2
for(var o=new TextEncoder,r=0;r<d;r++){var a=e.subDevices[r].mac
a&&(n[t++]=a.length,n.set(o.encode(a),t),t+=a.length)}var _=PROTOCOL_VER.concat(this._getMQTTLenArray(t)).concat(Array.from(n.slice(0,t)))
...
break
case"s2c_raw":var r=void 0,a=[],_=i.data.did.substr(0,LEN_DID),o=this._callbackObj._boundDevices[_]
if(o){for(var c=0,s=4;s<i.data.raw.length&&128&i.data.raw[s];s++)++c
CMD_TRANS_BUSINESS_RESP===i.data.raw[7+c]?(r=i.data.raw[12+c],a=i.data.raw.slice(13+c)):(r=i.data.raw[8+c],a=i.data.raw.slice(9+c)),P0_CMD_REPORT_SUBDEVICE_STATUS===r?(this._callbackObj._processSubdeviceOnlineReport(_,a),this._callbackObj._onUpdateSubDevices(_,!0)):P0_CMD_GET_SUBDEVICE_LIST_RESP===r||P0_CMD_REPORT_SUBDEVICE_LIST===r?(this._callbackObj._processSubdevicesReport(_,a),this._callbackObj._onUpdateSubDevices(_,!0)):P0_CMD_ADD_SUBDEVICE_RESP===r?a[0]?this._callbackObj.onUpdateSubDevices&&this._callbackObj._sendError(this._callbackObj.onUpdateSubDevices,GIZ_SDK_SUBDEVICE_ADD_FAILED,"add subDevice for center control device failed",_):this._callbackObj._onUpdateSubDevices(_,!1):P0_CMD_DELETE_SUBDEVICE_RESP===r?a[0]?console.log("center control device "+_+" delete subDevice failed"):console.log("center control device "+_+" delete subDevice success"):this._callbackObj.onReceiveData&&this._callbackObj.onReceiveData({did:o.did,raw:i.data.raw})}break
